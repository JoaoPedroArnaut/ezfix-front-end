...
const Input = ({ label, pattern, disabled, alternativoDois, placeholder, alternativo, size, type, id, onChange, value, onBlur, maxLength }) => {
    ...
    if (alternativo) {
        ...
        return (
            <div className={estilo}>
                <Label value={label + ":"} />
                <input pattern={pattern} disabled={disabled} maxLength={maxLength} id={id} value={value} onBlur={onBlur} type={type} onChange={onChange} className={disabled ? "bg-gray" : "bg-white"} className="w-full rounded-lg p-4 h-10 filter drop-shadow-md" placeholder={placeholder} />
            </div>
        )
