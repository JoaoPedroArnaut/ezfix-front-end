import React from 'react'
import Botao from '../components/Botao'
import Footer from '../components/Footer'
import Navbar from '../components/Navbar'

const login = () => {
    return (
        <>
            <Navbar login={["Login", "Criar conta"]} page="/" />
            <div className="w-full my-16 flex justify-around items-center">
                <div className="w-9/12 flex">
                    <div className="w-1/2 bg-blue-light border-2 border-r-0 border-solid border-opacity-10 border-black filter shadow-xl rounded-l-xl flex flex-col items-center p-8">
                        <img src="/undraw_secure_login_pdn4 1.svg" alt="" className="pr-8" />
                        <div className="flex flex-col items-center mt-8 text-lg font-medium">
                            <p>Ainda não tem uma conta?</p>
                            <p>Crie a sua agora!</p>
                        </div>
                        <Botao estilo={0} text="cadastro"/>
                    </div>
                    <div className="w-1/2 border-2 border-l-0 border-solid border-opacity-10 border-black filter shadow-xl rounded-r-xl flex flex-col items-center py-16">

                    </div>
                </div>
            </div>
            <Footer />
        </>
    )
}

export default login
